...
/* ======================================================================	*/
void sx1276_InitLoRaMode (sx1276_LoRa_Modem *modem, sx1276_LoRa_InitTypeDef *config)
{
	float f_freq;
	uint32_t i_freq;
	...
	// Основа основ - какой SPI используется и где CS
	modem->Control.CS_Pin = config->CS_Pin;
	modem->Control.CS_PORT = config->CS_PORT;
	modem->Control.SPIx = config->SPIx;
	...
	sx1276ReadRegs (&modem->Control, LORA_REGOPMODE, (uint8_t*)&modem->confRegs + 1, sizeof (sx1276_regs) - 1);		// Считываем на 1 меньше, т.к. регистр 0х00 - это ФИФО
	...
	sx1276WriteRegs (&modem->Control, LORA_REGOPMODE, (uint8_t*)&modem->confRegs + 1, sizeof (sx1276_regs) - 1);
	...
	modem->Control.curState = SX1276_STATE_IDLE;
	modem->Control.prevState = SX1276_STATE_IDLE;
}
/* ======================================================================	*/
...
/* ======================================================================	*/
void sx1276_LoRa_SetMode (sx1276_LoRa_Modem *modem, sx1276_lora_mode_t mode)
{
...
/* ======================================================================	*/
void sx1276_LoRa_SetFrequency (sx1276_LoRa_Modem *modem, uint32_t frequency)
{
	uint32_t i_freq;
	float f_freq;
	uint8_t tmp;
...
/* ======================================================================	*/
uint32_t sx1276_LoRa_GetFrequency	(sx1276_LoRa_Modem *modem)
{
	uint32_t i_freq;
	uint32_t f_freq;
...
/* Возвращаемое значение: нет																							*/
/* ======================================================================	*/
void sx1276_LoRa_sendPacket (sx1276_LoRa_Modem *modem, uint8_t *data, uint8_t length)
{
	modem->Control.prevState = modem->Control.curState;
	modem->Control.curState = SX1276_STATE_TX;
...
/* Возвращаемое значение: нет																							*/
/* ======================================================================	*/
void sx1276_LoRa_startRx (sx1276_LoRa_Modem *modem)
{
	modem->Control.curState = SX1276_STATE_RX;
...
/* Возвращаемое значение: нет																							*/
/* ======================================================================	*/
void sx1276_LoRa_stopRx (sx1276_LoRa_Modem *modem)
{
	modem->Control.curState = SX1276_STATE_IDLE;
...
/* Возвращаемое значение: нет																							*/
/* ======================================================================	*/
void sx1276_LoRa_updateState (sx1276_LoRa_Modem *modem)
{
	...
	if (tmp & LORA_INTFLAG_TX)
	{
		...
		if (modem->Control.prevState != SX1276_STATE_IDLE)
		{
	...
	if (tmp & LORA_INTFLAG_TIMEOUT)
	{
		// Долго долго ничего вдруг не принимали и взвесился флаг TIMEOUT. Перезапустим режим приема
		sx1276_LoRa_SetMode (modem, SX1276_LORA_MODE_RXCONTINUOUS);
		modem->Control.curState = SX1276_STATE_RX;
	}
}
...
/* ======================================================================	*/
uint8_t sx1276_LoRa_getRssi (sx1276_LoRa_Modem *modem)
{
...
/* ======================================================================	*/
/* Функция изменения выходной мощности трансивера                       	*/
/* Входные аргументы: *modem - указатель на объект LoRa modem							*/
/*                    pa_level - уровень выходного сигнала (1 - 15)       */
/* Возвращаемое значение: RSSI сигнала на текущей частоте									*/
/* ======================================================================	*/
void sx1276_LoRa_setPAlevel (sx1276_LoRa_Modem *modem, uint8_t pa_level)
{
	modem->confRegs.Reg09_PAConfig &=~ LORA_REG_PACONFIG_OUTPOWER_MASK;
	modem->confRegs.Reg09_PAConfig |= (pa_level);
	
	sx1276WriteReg (&modem->Control, LORA_REG_PACONFIG, modem->confRegs.Reg09_PAConfig);
}
