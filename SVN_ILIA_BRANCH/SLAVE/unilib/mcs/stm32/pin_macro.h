	...
	*/
	...
	{																											\
		...
		(GPIO##XPORT##)->OSPEEDR |=   ((uint32_t)(XSPEED) << ((uint32_t)(XPIN) * 2UL));							\
																												\
		(GPIO##XPORT##)->OTYPER &=~  ( 0x01 << (uint32_t)(XPIN) );												\
	...
	#define _PIN_SET(   XPORT, XPIN, XALEVEL, XMODE, XSPEED)  do {GPIO##XPORT->BSRRL = (1UL << (uint16_t)(XPIN)); } while (0)  // Atomic operation
	#define _PIN_CLR(   XPORT, XPIN, XALEVEL, XMODE, XSPEED)  do {GPIO##XPORT->BSRRH  = (1UL << (uint16_t)(XPIN)); } while (0)  // Atomic operation
	...
	#define OUTPUT_OPEN_DRAIN			((0x01<<4) | (0x01<<2) | (0x00))
