...
//======================================================================TEST!!!!!!!!!!!!!!!!!
/*
линейное включение интенсивности в соответствии со временем и положением светодиода по Y . ¬озвращает коеффициент;
fade - сколько светодиодов учавствует в плавном переходе. ћожет быть больше чем светодиодав в линие
*/
uint8_t get_linear_progress_y_TEST(uint8_t crosfade, uint16_t SIZE_LED_STACK, uint16_t led_index, uint8_t cw,uint16_t fade)
{
	...
	//номер первого в цепочке изменений пиксел€
	...
	if(cw)
	{
		...
		*/
		...
		if(led_index>front_pixel) fade_component = 0;
...
void linear_color_change_handler(frame instruction, uint16_t x, uint16_t y, leds *this_led, uint8_t crosfade)
{
		...
		{
			led_state = (float)get_linear_progress_y_TEST(crosfade, X_MATRIX_SIZE, x, this_linear_process->route,this_linear_process->fade)/255;// коефициент состо€ни€ светодиода
		}
		else
		{
			led_state = (float)get_linear_progress_y_TEST(crosfade, Y_MATRIX_SIZE, y, this_linear_process->route,this_linear_process->fade)/255;// коефициент состо€ни€ светодиода
		}
